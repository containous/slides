
[%notitle]
= Tales of the Kubernetes Ingress Networking: Deployment Patterns for External Load Balancers
include::./includes/attributes.adoc[]

[{invert}]
== !

[.title]
Tales of the Kubernetes Ingress Networking: Deployment Patterns for External Load Balancers

image::kubernetes.svg["Traefik's Logo",height=200]

[{invert}]
== How to Access the Slides?

image::qrcode.png["QRCode to this presentation",height=150]

[.small]
- link:{presentationUrl}[Slides (HTML): {presentationUrl}]
- link:{presentationUrl}/slides.pdf[Slides (PDF): {presentationUrl}/slides.pdf]
- link:{repositoryUrl}[Source on icon:github[]: {repositoryUrl}]

== How to Use the Slides?

* *Browse the slides:* Use the arrows
** Change chapter: Left/Right arrows
** Next or previous slide: Top and bottom arrows

* *Overview of the slides:* keyboard's shortcut "kbd:[o]"

* *Speaker mode (and notes):* keyboard's shortcut "kbd:[s]"

// //// Speaker Slide and Company slide

== Whoami

* Manuel Zapf:
** Head of Product Open Source @ link:https://containo.us/[Containous,window=_blank]
* link:https://twitter.com/mZapfDE[icon:twitter[] @mZapfDE,window=_blank]
* link:https://github.com/SantoDE[icon:github[] SantoDE,window=_blank]

image::manuel.jpg[height=200]

[{invert}]
== Containous

link:https://containo.us[]

* We Believe in Open Source
* We Deliver Traefik, Traefik Enterprise Edition and Maesh
* Commercial Support
* 30 people distributed, 90% tech

image::containous-logo-black.png["Containous Logo", height=200]

// //// end of Speaker Slide and Company slide


// ///////////////////////
// Include chapter here with a blank line between each
// Notice the offset to have chapters as autonomous documents
// ///////////////////////
:leveloffset: +1

include::./includes/kube-101.adoc[]

include::./includes/notes-about-ingresses.adoc[]

include::./includes/external-lbs.adoc[]

include::./includes/source-ip-on-kingdom-kube.adoc[]

include::./includes/demos.adoc[]

:leveloffset: -1

== Sources

* link:https://kubernetes.io/docs/tutorials/services/source-ip/[]
* link:https://en.wikipedia.org/wiki/Network_address_translation[]
* link:https://www.asykim.com/blog/deep-dive-into-kubernetes-external-traffic-policies[]

[{invert}]
== Read More

image::microservices-architecture-white-paper.png["Containous Logo", height=400]

link:https://info.containo.us/request-white-paper-routing-in-the-cloud[]


[{invert}]
== Thank you!



link:https://twitter.com/mZapfDE[icon:twitter[] @mZapfDE]

link:https://github.com/SantoDE[icon:github[] SantoDE]

image::qrcode.png["QRCode to this presentation",height=150]

[.small]
- link:{presentationUrl}[Slides (HTML): {presentationUrl}]
- link:{presentationUrl}/slides.pdf[Slides (PDF): {presentationUrl}/slides.pdf]
- link:{repositoryUrl}[Source on icon:github[]: {repositoryUrl}]
