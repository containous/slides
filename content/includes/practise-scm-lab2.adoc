...
**Solution:** Do not remove the prefix (Thanks Captain Obvious!) in the ingress rule in `lab2-gitea.yml`.
...
**Solution:** Use (once again again) a service linked to an external address in `lab2-gitea.yml`.
...
[source,bash,subs="attributes"]
----
kubectl apply -f ./lab2-gitea.yml
----
